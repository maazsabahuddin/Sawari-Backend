...
def verify_hmac_sha1(request, client_secret=None,
    ...
    if not match:
        log.debug('User HMAC-SHA1 failed: signature base string: %s',
                  sig_base_str)
    return match
...
def verify_hmac_sha256(request, client_secret=None,
    ...
    match = safe_string_equals(signature, request.signature)
    if not match:
        log.debug('User HMAC-SHA256 failed: signature base string: %s',
                  sig_base_str)
    return match
...
def verify_rsa_sha1(request, rsa_public_key):
    """User a RSASSA-PKCS #1 v1.5 base64 encoded signature.
    ...
    if not verify_ok:
        log.debug('User RSA-SHA1 failed: signature base string: %s',
                  sig_base_str)
    return verify_ok


def verify_plaintext(request, client_secret=None, resource_owner_secret=None):
    """User a PLAINTEXT signature.
    ...
    match = safe_string_equals(signature, request.signature)
    if not match:
        log.debug('User PLAINTEXT failed')
    return match
